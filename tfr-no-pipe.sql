...
-- tracefile-no-pipe.sql

-- retrieve the trace file without creating db objects
...
begin
	...
	dbms_lob.createtemporary(lob_loc => trace_data, cache => true, dur => dbms_lob.call);

	while true
	loop
		begin
			utl_file.get_line(fh_tracefile, buf, 32767);
			c_tmp_clob := to_clob(buf);
			c_tmp_clob := c_tmp_clob || chr(10);
			dbms_lob.append(trace_data,c_tmp_clob);
			--pl(buf);
		exception
	...
	--pl('i_elapsed_seconds: ' || to_char(i_elapsed_seconds));
